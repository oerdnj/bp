...
add\_hash.py přidán unikátní identifikátor SHA-1 (viz výše). V~dalším
kroku byly pomocí skriptu get\_random.py vybrány náhodné inzeráty pro
pilotáž a~tyto inzeráty byly následně z~hlavních datových souborů
odstraněny. Po dokončení pilotáže byl pomocí skriptu get\_random.py
vybrán hlavní výzkumný vzorek, který byl uložen opět jako soubor CSV.
Tentokrát již byly použity pouze soubory dva -- kódování probíhalo
samostatně pro muže a~pro ženy.  Tyto soubory byly naimportovány do
tabulkového procesoru OpenCalc z~kancelářské sady
OpenOffice.org. Kódování bylo prováděno dvoufázově. V~první fázi byla
kódována pouze klíčová slova do dvou samostatných sloupců s~poptávkou
a~nabídkou. Výsledek tohoto prvního kola byl vyexportován opět do
formátu CSV a~následně zpracován pomocí skriptu code.py, který
automaticky překódoval použitá klíčová slova do vybraných kategorií
(závazek, atraktivita, status/majetek, atd.).  Výsledná kontingenční
tabulka byla uložena do jednoho výsledného souboru CSV, který byl
naimportován do programu SPSS 18.0.
...
poptávají sociální schopnosti.  Z~celkového počtu ženami poptávaných
sociálních schopností ženami (N=170) jich více než polovina (N=93)
obsahuje poptávku po ,,smyslu pro humor``.
...
\ctable[
  ...
  ~~Poptávaný    &   6 &  1,6\% &   5 &  1,4\% &  0,000~~~\LL
}
